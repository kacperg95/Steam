...
{
    public class GetFullListOfGames(IServiceProvider _serviceProvider, ILogger<GetFullListOfGames> _logger) : BackgroundService
    {
        ...
        {
            using var _logScope = _logger.BeginScope(new Dictionary<string, object> { ["BackgroundWorkerName"] = nameof(GetFullListOfGames) });
            while (!stoppingToken.IsCancellationRequested)
            {
                while (GetLatelyPopularGames.IsRunning || UpdateGames.IsRunning)
                {
                    _logger.LogInformation("Waiting for other background workers to finish before starting monthly game sync");
                    await Task.Delay(TimeSpan.FromMinutes(30), stoppingToken);
                }

                IsRunning = true;
                _logger.LogInformation("Initializing monthly game sync");
                ...
                try
                {
                    ...
                    while (!stoppingToken.IsCancellationRequested)
                    {
                        using (var scope = _serviceProvider.CreateScope())
                        {
                            ...
                            var result = await _steamApiClient.GetGameIds(page, false);
                ...
                {
                    _logger.LogError(ex, "GetFullListOfGames error");
                }

                IsRunning = false;
                _logger.LogInformation("Monthly game sync successful");
